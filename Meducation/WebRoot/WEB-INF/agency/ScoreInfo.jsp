...
<script>
  $(function() {
    $( document ).tooltip({
      ...
      items: "label.infom",
      content: function() {
          return "<div>"+"<p class='header'><strong>登记课程成绩操作说明：</strong></p>"+
          "<p class='infor'>1、页面列出了当前全部正在执行的课程，用户可根据关键词筛选出需要添加成绩的课程；</p>"
          +"<p class='infor'>2、点击“登记成绩”按钮，将看到该课程的学员名单，并可以为学员记录成绩；</p>"+
          "</div>";
				...
				${(pageList.pageNumber-1)*pageList.objectsPerPage+table_rowNum} 
						...
						<display:column property="courseCode" title="课程编号" sortable="true"
			 			...
			 			<display:column title="编辑" media="html" style="text-align:center;">
							<img src="./img/MenuIcon/FUNC20029.gif"/><a href="TeacherEnrollment_score.action?AgencyId=${table.agencyId}&CourseId=${table.courseId}&page=${pageList.pageNumber}" style="color:red;font-weight:bold;">登记成绩</a>&nbsp;
			              </display:column>
			<%----%>			
...
<div class="savetishi"><img src="img/function/help.png" style="width:15px;height:15px;"/>&nbsp;<label class="infom">登记课程成绩操作说明</label></div>
</div>
  </body>
</html>
