 ...
 $(function() {		
	$('#back').click(
		...
		if(${sessionScope.Source}=='1')		
         window.location="PlanSchool_showall.action?PlanId=${sessionScope.G_PlanId}"; 
         else if(${Source}=='2')
	...
	<table>
	  <tr>					
        <td style="width:1500px;text-align:right;"><button type="button" class="button black"
			...
			<div id="displayTable">
					<display:table name="pageList" id="table" export="true" requestURI="PlanSchool_look.action"
				...
				${(pageList.pageNumber-1)*pageList.objectsPerPage+table_rowNum} 
						...
						<display:column property="planNum" title="培训计划编号" sortable="true"
							headerClass="sortable" style="text-align:center;" />
						<display:column property="belongDistrictName" title="学校所属区县" sortable="true"
							headerClass="sortable" style="text-align:center;" />
						<display:column property="schoolName" title="学校名称" sortable="true"
							headerClass="sortable" style="text-align:center;" />
						<display:column property="schoolCode" title="学校编码" sortable="true"
							headerClass="sortable" style="text-align:center;" />
						...
						<display:column title="编辑" media="html" style="text-align:center;">
							<s:url action="PlanSchool_querydel" id="delcity">
							...
							<img src="./img/MenuIcon/FUNC20029.gif"/><a href="${delcity}" onclick="return window.confirm('您真的想删除本条记录吗？删除后将不可恢复！')">删除</a>&nbsp;
			                 ...
			                 <s:else><img src="./img/MenuIcon/FUNC20029.gif"/><label class="seeinform">不可编辑</label></s:else>
                        </display:column>
